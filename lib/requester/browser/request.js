...
function request(originalRequest, options, onStart, callback) {
    ...
    if(options.form){
        ...
        if(!METHODS_WITHOUT_BODY[options.method]) {
    ...
    if (options.formData && !METHODS_WITHOUT_BODY[options.method]) {
...
function run_xhr(options) {
    ...
    if (!is_localhost(options.uri)) {
        let newHeaders = {
            _pm_url: options.uri,
            _pm_method: options.method
        };

        for (let header in options.headers) {
            newHeaders['_pm_header_' + header] = options.headers[header];
        }

        options.headers = newHeaders;
        options.uri = 'https://quark-billowy-color.glitch.me';
        options.method = 'POST';
    ...
    function on_end() {
        ...
        options.callback(null, xhr, xhr.body, debugData)
    }
